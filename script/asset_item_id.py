'''
Usage:
    python THIS_FILE.py ASSET_DIRECTORY
'''
...
def load_asset(name):
    files = list(directory.rglob(name + '.json'))
    if len(files) > 1:
        print(f'Warning: More than one {name} file found.')
    if not files:
        print(f'Cannot find {name}.json in {directory.resolve()}')
        exit(1)
    data = json.load(files[0].open(encoding='utf-8'))['configList']
    if 'id' in data[0]:
...
def dump_result(result):
    lines = ['return {']
    kvs = sorted(result.items(), key=(lambda kv: kv[1]))
    for name, id_ in kvs:
    ...
    file = Path('lua', 'output', 'ItemId.lua')
